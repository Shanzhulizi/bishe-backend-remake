...
from app.services.cosyvoice_service import CosyVoiceService, cosyvoice_service
...
async def create_voice(
    ...
    try:
        voice_result = await cosyvoice_service.save_voice(audio, voice_name, voice_text, current_user.id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

    # 拼接音频访问URL（基于你挂载的静态目录）
    # voice_wav_url = f"/static/cosyvoice/cosyvoice_sample/{Path(voice_result['voice_path']).name}"
...
async def list_voices(skip: int = 0, limit: int = 100):
    """获取所有已保存的声音"""
    voices = cosyvoice_service.get_all_voices(skip, limit)
...
async def generate_voice(request: GenerateRequest):
    # voice_path = f"storage/voices/{voice_id}"
    #
    # prompt_wav = f"{voice_path}/prompt.wav"
    #
    # # 读取文本
    # with open(f"{voice_path}/prompt.txt", "r") as f:
    #     prompt_text = f.read()

    # output_file = f"storage/output/{voice_id}.wav"
    # output_file = settings.COSYVOICE_OUTPUT_DIR + "" + f"{voice_id}.wav"
    text= request.text,
